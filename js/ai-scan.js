...
SC.renderAIScan=function(){SC.pushHistory?.("renderAIScan");if(!SC.currentUser||SC.currentProfile?.role!=="student"){SC.renderAIScanGuestNotice();return}document.getElementById("app").innerHTML=`<section class="card"><h1>📷 AIスキャン</h1><p class="help">写真を選んで読み取り、内容を確認して保存します。</p><div class="box"><label>種類</label><select id="scanType"><option>模試</option><option>定期テスト</option><option>通知表</option><option>共通テスト自己採点</option></select><label>画像</label><input id="scanImageInput" type="file" accept="image/*" capture="environment" onchange="SC.previewScanImage(event)"><div id="scanPreview" style="display:none"></div><div class="actions"><button class="btn navy" onclick="SC.runOCRScan()">画像を読み取る</button><button class="btn light" onclick="SC.renderOCRSettings()">OCR設定</button></div><p id="ocrStatus" class="help">${SC.ocrSpaceApiKey?"OCRキー設定済み":"OCRキー未設定：手入力可能"}</p></div><div class="box"><h2>読み取り結果</h2><textarea id="ocrRawText" placeholder="OCR結果がここに入ります"></textarea><button class="btn light" onclick="SC.autoFillFromOCRText()">フォームへ反映</button></div><div class="box"><h2>確認して保存</h2><label>テスト名</label><input id="testName"><label>実施日</label><input id="testDate" type="date" value="${new Date().toISOString().slice(0,10)}"><label>英語</label><input id="scoreEnglish" type="number"><label>数学</label><input id="scoreMath" type="number"><label>国語</label><input id="scoreJapanese" type="number"><label>理科</label><input id="scoreScience" type="number"><label>社会</label><input id="scoreSocial" type="number"><label>偏差値</label><input id="testDeviation" type="number" step="0.1"><label>判定</label><select id="testJudgement"><option></option><option>A判定</option><option>B判定</option><option>C判定</option><option>D判定</option><option>E判定</option></select><label>メモ</label><textarea id="testMemo"></textarea><div class="actions"><button class="btn primary" onclick="SC.saveTestRecord()">保存</button><button class="btn light" onclick="SC.renderMyTestRecords()">保存済みを見る</button></div><p id="testSaveMsg" class="help"></p></div></section>`;SC.updateBackButton?.()};
SC.previewScanImage=function(e){const f=e.target.files?.[0],box=document.getElementById("scanPreview");if(!f||!box)return;const url=URL.createObjectURL(f);box.style.display="block";box.innerHTML=`<img class="previewImg" src="${url}">`};
SC.runOCRScan=async function(){const st=document.getElementById("ocrStatus"),raw=document.getElementById("ocrRawText"),file=document.getElementById("scanImageInput").files?.[0];if(!file){st.textContent="画像を選択してください。";return}if(!SC.ocrSpaceApiKey){st.textContent="OCRキー未設定です。OCR設定から入力してください。";return}try{st.textContent="読み取り中...";const form=new FormData();form.append("apikey",SC.ocrSpaceApiKey);form.append("language","jpn");form.append("OCREngine","2");form.append("isOverlayRequired","false");form.append("file",file);const res=await fetch("https://api.ocr.space/parse/image",{method:"POST",body:form});const data=await res.json();if(data.IsErroredOnProcessing)throw new Error(data.ErrorMessage||"OCRエラー");raw.value=(data.ParsedResults||[]).map(r=>r.ParsedText||"").join("\n").trim();st.innerHTML=`<span class="ok">読み取り完了。</span> 内容を確認してください。`;SC.autoFillFromOCRText()}catch(e){st.innerHTML=`<span class="warn">読み取り失敗。</span> 手入力で保存できます。`;console.error(e)}};
